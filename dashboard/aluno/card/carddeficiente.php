...
$sql = "
    ...
    WHERE a.escola_id IN ($escola_placeholders) 
      AND a.classe_id IN ($classe_placeholders) 
      AND a.deficiente = 1
    ORDER BY e.nome ASC, t.nome_turma ASC
";
...
$sql_feminino = "
    SELECT COUNT(*) as total_feminino
    FROM aluno a
    WHERE a.escola_id IN ($escola_placeholders) 
      AND a.classe_id IN ($classe_placeholders) 
      AND a.deficiente = 1 
      AND a.genero = 'Feminino'
";
...
<div class="dashboard-tables-charts">
    <div class="table-card">
        <h3 class="table-title">
            Lista dos(as) Alunos(as) <span class="highlight">Deficientes</span> 
            ...
            <tbody>
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        ...
                        echo "<td>" . htmlspecialchars($row['tipo_deficiencia'] ?? '') . "</td>";
                        echo "</tr>";
                    }
                } else {
                    echo "<tr><td colspan='14'>Nenhum aluno encontrado.</td></tr>";
                }
                ?>
            </tbody>
        </table>
