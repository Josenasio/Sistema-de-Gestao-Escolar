...
$sql = "
    SELECT 
        ...
        a.data_conhecimento_gravidez,
         a.observacao_gravidez,
    ...
    WHERE a.escola_id = ? AND a.gravidez = 1
...
if ($result->num_rows > 0) {
    echo "<h2 class='text-center text-primary my-5'>Lista de Alunas Grávidas</h2>";
...
<th>Observaçao</th>
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        echo "<td contenteditable='true' onblur='salvarEdicao(this, " . $row['id'] . ", \"observacao_gravidez\")'>" . $row['observacao_gravidez'] . "</td>";

        echo "<td contenteditable='true' onblur='salvarEdicao(this, " . $row['id'] . ", \"data_conhecimento_gravidez\")'>" . $row['observacao_gravidez'] . "</td>";
...
function salvarEdicao(elemento, id, campo) {
    ...
    xhr.open("POST", "salvar_gravida.php", true);
...
function exportToExcel() {
    ...
    XLSX.writeFile(wb, 'Lista_gravidas.xlsx');
}
</script>