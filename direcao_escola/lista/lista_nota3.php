...
<body class='bg-light' style='background-color: #c1efde;'>
    ...
    <div class='container'>
        <h1 class='my-4 text-center'>Lista das Notas do 3º Periodo</h1>";
...
foreach ($alunos as $grupo) {
    ...
    foreach ($tabela_alunos as $key => $notas_por_disciplina) {
        ...
        foreach ($disciplinas as $disciplina) {
            $nota = $notas_por_disciplina[$disciplina] ?? ['nota5' => '-', 'nota6' => '-', 'nota_final2' => '-'];

            echo "<td class='" . ($nota['nota5'] < 10 ? 'nota-vermelha' : '') . "'>{$nota['nota5']}</td>";
            echo "<td class='" . ($nota['nota6'] < 10 ? 'nota-vermelha' : '') . "'>{$nota['nota6']}</td>";
...
function exportToExcel() {
    ...
    XLSX.writeFile(wb, 'Lista_nota_3ºPeriodo.xlsx');
}
</script>