...
<body class='bg-light' style='background-color: #c1efde;'>
    ...
    <div class='container'>
        <h1 class='my-4 text-center'>Lista das Notas do 1º Periodo</h1>";
...
foreach ($alunos as $grupo) {
    ...
    foreach ($tabela_alunos as $key => $notas_por_disciplina) {
        ...
        foreach ($disciplinas as $disciplina) {
            $nota = $notas_por_disciplina[$disciplina] ?? ['nota1' => '-', 'nota2' => '-', 'nota_final1' => '-'];

            echo "<td class='" . ($nota['nota1'] < 10 ? 'nota-vermelha' : '') . "'>{$nota['nota1']}</td>";
            echo "<td class='" . ($nota['nota2'] < 10 ? 'nota-vermelha' : '') . "'>{$nota['nota2']}</td>";
            echo "<td class='" . ($nota['nota_final1'] < 10 ? 'nota-vermelha' : '') . "'>{$nota['nota_final1']}</td>";
        }
        echo "</tr>";
    }
...
function exportToExcel() {
    ...
    XLSX.writeFile(wb, 'Lista_nota.xlsx');
}
</script>