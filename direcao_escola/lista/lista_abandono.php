...
if (!isset($_SESSION['id_escola'])) {
    die("Você não está logado. Acesso negado.");
}
...
$sql = "
    ...
    WHERE a.escola_id = ? AND a.motivo_abandono IS NOT NULL   AND a.motivo_abandono <> ''
...
if ($result->num_rows > 0) {
    echo "<h2 class='text-center text-primary my-5'>Lista de Alunos que Abandonaram a Escola</h2>"; 
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        echo "<td contenteditable='true' onblur='salvarEdicao(this, " . $row['id'] . ", \"motivo_abandono\")'>" . $row['motivo_abandono'] . "</td>";
        echo "<td contenteditable='true' onblur='salvarEdicao(this, " . $row['id'] . ", \"estrategia_recuperacao\")'>" . $row['estrategia_recuperacao'] . "</td>";

        echo "<td>" . $row['nome_classe'] . "</td>";
        echo "<td>" . $row['nome_turma'] . "</td>";
        echo "<td><button class='btn btn-primary' onclick='editarAluno(" . $row['id'] . ")'>Editar</button></td>";
        echo "</tr>";
    }
...
function salvarEdicao(elemento, id, campo) {
    ...
    xhr.open("POST", "salvar_abandono.php", true);
...
<!-- Botão fixo de "Voltar a Pagina Inicial" -->
...
function exportToExcel() {
    ...
    XLSX.writeFile(wb, 'Lista_abandono.xlsx');
}
</script>